...
# -------------------------------------------------------------
# WON COUNTER (with companies list)
# -------------------------------------------------------------
def show_won_counter(conn: sqlite3.Connection):
    ...
    if companies:
        pills_html = "".join(
            ...
            f'border-radius:999px;background:#444;color:#fff;font-size:11px;">'
            f'{c}</span>'
            for c in companies
        )
        companies_html = f"""
            <div style="margin-top:4px;font-size:11px;opacity:0.9;text-align:right;">
                Sold to:<br>{pills_html}
            </div>
        """
    else:
        companies_html = """
